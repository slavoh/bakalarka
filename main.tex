...
We have created a compilation of existing algorithms for generating from a uniform distribution on a polyhedron with a comparison on random polyhedra. As possible algorithms, we have considered Gibbs sampler and Hit--and--Run sampler from Metropolis--Hastings class and the rejection method using the minimum volume enclosing ellipsoid as a superset. As a score function, we have used the expected generation time of a large number of points for a fixed dimension of space. We have shown that the quickest algorithm for doing so was Hit--and--Run generator. In addition, we have introduced optimal design problem with algorithms Vertex Exchange Method and Randomized Exchange Algorithm as its solvers. We have shown, how could equivalence of optimal design problem and minimum volume enclosing ellipsoid problem be used for finding minimum volume enclosing ellipsoid for given polyhedra. Moreover, we have discussed random polyhedra generation. We have shown that just defining ``a random polyhedron'' is non--trivial problem.


\paragraph*{Keywords: Metropolis--Hastings, Hit--and--Run, Gibbs, rejection methods, minimume volume enclosing ellipsoid, optimal design problem, random polyhedra} 
